...
cap1 = cv2.VideoCapture(0)  # 웹캠 인덱스 (0부터 시작) 설정
...
def generate_virtual_frame():
    ...
    partA = BODY_PARTS["RHip"]   #오른골반
    partB = BODY_PARTS["RKnee"]   #오른무릎
    partC = BODY_PARTS["RAnkle"]    #오른발목

    partX = BODY_PARTS["RKnee"] #오른무릎
    partY = BODY_PARTS["RHip"]  #오른골반
    partW = BODY_PARTS["Chest"] #가슴
    ...
    if points[partC] != None:
        if points[partW] != None:
            ...
            if isinstance(points[partA], tuple) and isinstance(points[partB], tuple):
                x1, y1 = points[partA] #오른골반 좌표
                x2, y2 = points[partB] #오른무릎 좌표
                x3, y3 = points[partC] #오른발목 좌표
                ...
                # 각도가 170미만 190초과라면 잘못된 자세라는 판정으로 선을 빨간색으로 변경
                if angle > 190:
                ...
                if isinstance(points[partX], tuple) and isinstance(points[partY], tuple):
                    a1, b1 = points[partX] #오른무릎
                    a2, b2 = points[partY] #오른골반
                    a3, b3 = points[partW] #가슴
                    ...
                    # 각도가 100미만 150초과라면 잘못된 자세라는 판정으로 선을 빨간색으로 변경
                    if angle_2 > 150: #빨강
