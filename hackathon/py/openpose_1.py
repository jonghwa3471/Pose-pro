...
def generate_virtual_frame():
    ...
    partA = BODY_PARTS["RWrist"]   #오른손목
    partB = BODY_PARTS["RShoulder"]   #오른어깨
    partC = BODY_PARTS["RHip"] #오른골반
    ...
    if points[partC] != None:
        if isinstance(points[partA], tuple) and isinstance(points[partB], tuple):
            ...
            # 각도가 60미만 90초과라면 잘못된 자세라는 판정으로 선을 빨간색으로 변경
            if angle > 90: 
                cv2.line(frame, points[partA], points[partB], (0, 0, 255), 2) #(B,G,R)
